...
{{ end }}
  ...
  # scheduled.
  prometheus-serve-addr: ":{{- or .Prometheus.AgentPort "9090" }}"
  {{ end }}
  ...
  install-iptables-rules: "{{- if .InstallIPTablesRules -}}true{{- else -}}false{{- end -}}"
...
{{ with .Networking.Cilium }}
        ...
        {{ if .Prometheus.Enable }}
        ports:
        - containerPort: {{ or .Prometheus.AgentPort "9090" }}
          hostPort: {{ or .Prometheus.AgentPort "9090" }}
          name: prometheus
          protocol: TCP
        {{ end }}
        readinessProbe:
